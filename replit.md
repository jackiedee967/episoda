...
- **User Mention System**: Production-ready @mention tagging in posts/comments with autocomplete dropdown, clickable mention links, and notifications. Features fully controlled input model with cursor delta calculation for reliable mention detection in all scenarios (typing at end, mid-text edits, prefills, suggestion insertion). Optimized for performance with 300ms debouncing, follow list caching, and single-query mutual friends calculation via SQL RPC function. Database schema includes `comment_mentions`, `post_mentions`, and `notifications` tables. Mention autocomplete sorts by: following > mutual friends > alphabetical.
