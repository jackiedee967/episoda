...
- **Favorites Section**: Profile feature allowing users to showcase up to 3 favorite shows. Displays below "Currently Watching" section with dotted-border placeholder boxes (+ icon) for empty slots and show posters for filled slots. Own profile shows all 3 slots with add/remove capability; other user profiles hide section when empty. Uses `user_favorites` table with display_order column (1-3). Reuses existing "Select a Show" modal pattern with search and recommendations. X remove button appears at bottom-right of posters (own profile only). Implements Supabase `as any` workaround for PostgREST schema cache issue.
