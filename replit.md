...
- **FEATURE - 4-Tier Recursive Comment Nesting**: Implemented unlimited comment nesting (up to 4 visible tiers) with recursive rendering, progressive visual hierarchy, and full like/reply support at all levels. Unified Comment/Reply types into single recursive Comment interface, refactored CommentCard for recursive self-rendering with depth-based avatar sizing (30px→24px→20px→16px) and left indentation (8px per level). Updated post detail page to build recursive comment trees from `parent_comment_id` relationships. All like/reply handlers now work recursively using `findCommentById()` and `findAndUpdateComment()` helpers.
- **CRITICAL FIX - Post Creation Data Type Fix**: Fixed PostgreSQL "value out of range for type integer" errors by aligning episode save logic with database schema. Episode IDs are VARCHAR (auto-generated by Supabase via composite key `show_id + season_number + episode_number`). PostModal now passes `null` for `trakt_id` when Trakt data unavailable, letting database handle ID generation. Updated `TraktEpisode.ids.trakt` type to `number | null` to support fallback scenarios.
- **CRITICAL FIX - ShowHub Navigation**: Fixed "Log an Episode" button opening "Select a Show" popup instead of rating page. ShowHub now passes `preselectedEpisodes` to PostModal, allowing users to select episodes in ShowHub and go directly to rating screen. Added database fallback for episode loading when Trakt fails.
- **CRITICAL FIX - Episode Reuse Logic**: PostModal fetches all existing episodes ONCE before save loop (not N times per iteration), reuses existing episodes by matching season/episode numbers, and aborts post creation if any episode fails to save (ensuring data consistency).
...
- **VISUAL FIX - Profile Picture Border Radius**: Updated Post Page profile pictures with new border radii: top corner (12px), top-level comments (30×30px/10px), nested comments (24×24px/8px).
