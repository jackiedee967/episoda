...
- **Genre Discovery System**: Three-view routing architecture: (1) Explore page with genre grid, (2) Genre detail pages showing popular shows with nano-genre filter pills, (3) Nano-genre filtered results. Features 90 curated nano-genres (Murder Investigation, Time Travel, Post-Apocalyptic, etc.) mapped to parent genres with emojis and TMDB keyword IDs. Uses official TMDB TV genre IDs (10759 Action, 16 Animation, 35 Comedy, 80 Crime, 99 Documentary, 18 Drama, 10751 Family, 10762 Kids, 9648 Mystery, 10763 News, 10764 Reality, 10765 Sci-Fi & Fantasy, 10766 Soap, 10767 Talk, 10768 War & Politics, 37 Western). Fallback genres (Music→Drama, Horror→Mystery, Thriller/Romance/History→Drama) use keyword filtering for precision. TMDB Discover API integration with genre+keyword filtering. **LIMITATION (Nov 21, 2025)**: Only 7 nano-genres have TMDB keyword IDs populated (time-travel, post-apocalyptic, superhero, zombie, vampire, coming-of-age, anime). UI now hides nano-genre pills without keyword IDs to prevent user confusion. Remaining ~83 nano-genres need keyword ID research via TMDB API before they can be enabled.
