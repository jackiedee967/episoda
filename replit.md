...
- **Recommendation Algorithm (Nov 23, 2025)**: Production-grade system with strict universal hard filters preventing irrelevant cross-genre/vintage content. **Centralized Filtering**: `applyHardFilters()` in recommendationScoring.ts enforces: (1) Primary genre MUST match (first genre in seed show's genres array), (2) For multi-genre shows require ≥2 genre overlap, (3) Must be within ±5 years of seed show. **Candidate Pool**: Fetches 100 Trakt related shows (increased from 30) for broader discovery. **Scoring Algorithm**: Weighted similarity matching - genres 50%, keywords 15%, demographics 15%, era 10%, rating 10%; minimum score threshold 20. **Display**: Shows 3+ recommendations per section (lowered from 6), no artificial limits on ranked results. **Section Detail Pages**: "Because You Watched" sections link to full paginated views with 3-column grid layout, infinite scroll, and all filtered recommendations. **Production Validation (Nov 23, 2025)**: Logs confirm filters working correctly (I Love LA: 100→25 candidates, showing modern comedies like Trentenaires, Freeridge; filters out anime, foreign, and vintage content from 2013-2018).
