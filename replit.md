...
- **CRITICAL FIX - ShowHub Navigation**: Fixed "Log an Episode" button opening "Select a Show" popup instead of rating page. ShowHub now passes `preselectedEpisodes` to PostModal, allowing users to select episodes in ShowHub and go directly to rating screen (line 864 in app/show/[id].tsx).
