...
- **CRITICAL FIX - Post Creation Resilience**: Fixed PostModal to handle Trakt API failures gracefully during post creation. When Trakt data is unavailable, PostModal validates and saves episodes using ShowHub data with collision-free episode IDs. Uses dual-path ID generation: Trakt-based formula (`showTraktId * 100000000 + season * 10000 + episode`) when trakt_id available, or UUID hash fallback (`hash(showUUID + season + episode)`) when trakt_id is null. Episode reuse logic prevents UNIQUE constraint violations by fetching existing episodes once and matching by season/episode numbers. Episode save failures are now fatal and abort post creation for data consistency. Posts succeed even when Trakt is down OR show lacks trakt_id.
- **CRITICAL FIX - Console Log Placement**: Fixed false "BLOCKED: Rating is 0" console messages that appeared even when rating validation passed. Moved console.log inside the if block to only log when rating is actually 0.
- **CRITICAL FIX - ShowHub Episode Logging**: Fixed PostModal regression where clicking an episode in ShowHub opened "Select a Show" screen instead of going directly to the rating page. PostModal now immediately shows the rating page when episodes are preselected from ShowHub, while loading Trakt data non-blocking in the background for post creation. Both guard clauses prevent step resets on Trakt failures, ensuring users stay on the rating page.
- **CRITICAL FIX - Resilient Episode Loading**: Implemented production-grade Supabase + TVMaze fallback system for ShowHub episode display. When Trakt API fails, ShowHub tries TVMaze to load ALL episodes, then falls back to database-only (logged episodes). Episodes use consistent Trakt-style IDs (`${traktId}-S${season}E${episode}`) to ensure logged episode detection, smart season expansion, and navigation work correctly. ALL episodes now display when possible, logged episodes ALWAYS display as final fallback.
- **Post Page Redesign**: Completed pixel-perfect redesign with poster on left (85×128px), user avatar top right, info column layout (timestamp → username "watched" → tags → rating), and divider. Fixed FadeInImage component to support containerStyle prop for wrapper dimensions.
