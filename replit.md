...
- **Recommendation Algorithm (Nov 23, 2025)**: Production-grade system with strict universal hard filters preventing irrelevant cross-genre/vintage/foreign/animated/low-quality content. **CRITICAL FIX**: Filter execution order ensures animation/language/rating checks ALWAYS apply before genre checks, preventing bypass via missing metadata. **Centralized Filtering**: `applyHardFilters()` in recommendationScoring.ts enforces (in order): (1) Animation/anime blocked unless seed is animated, (2) Language must be English (en) OR country in [us, gb, ca, au] - rejects if EITHER field is non-English, (3) Minimum Trakt rating ≥6.5, (4) Primary genre MUST match, (5) Multi-genre shows require ≥2 genre overlap, (6) Must be within ±5 years. **Candidate Pool**: Fetches 100 Trakt related shows for broader discovery. **Composite Ranking**: finalScore = similarity (60%) + popularity (40%), where popularity = normalized votes (70%) + rating (30%) - surfaces mainstream shows first. **Similarity Scoring**: Weighted matching - genres 50%, keywords 15%, demographics 15%, era 10%, rating 10%; minimum threshold 15 (lowered from 20). **Display**: Shows 3+ recommendations per section, ordered by composite score. **Section Detail Pages**: "Because You Watched" sections link to full paginated views with 3-column grid layout, infinite scroll, and all ranked recommendations. **Production Validation (Nov 23, 2025)**: Architect-approved zero-bypass filter stack - ALL foreign/anime/low-rated shows blocked even when metadata missing.
