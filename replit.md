...
- **Streaming Providers Display**: ShowHub displays streaming service logos (Netflix, Hulu, etc.) below the metadata line using TMDB Watch Providers API. Fetches flatrate/free/ads providers, deduplicates, and shows up to 5 logos in rounded rectangle containers. State properly resets between shows to prevent stale data.
